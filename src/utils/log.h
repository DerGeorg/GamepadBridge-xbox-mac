...
namespace Log
{
    ...
    #else
    // Compiled out unless DEBUG is defined. Parameters are left unnamed so
    // the no-op build stays warning-clean under -Wall -Wextra.
    inline void debug(std::string) {}

    template<typename... Args>
    inline void debug(std::string, Args...) {}
    #endif
