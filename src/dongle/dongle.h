...
public:
    ...
    // Thread-safe request to enter pairing mode. The actual work runs on the
    // USB thread to keep all libusb access single-threaded (see usb.h).
    void enablePairing();
...
private:
    ...
    // Single thread that owns all USB I/O: reads, writes and posted tasks.
    void usbThreadMain();

    std::thread usbThread;
    std::atomic<bool> stopThread;

    // Received packets, filled by the async reader callbacks and drained by
    // the USB thread. Touched only from the USB thread, so it needs no lock.
    std::queue<Bytes> inbound;

    // Work posted from other threads (e.g. pairing on a signal), executed on
    // the USB thread.
    std::mutex taskMutex;
